...
% Coefficients from huang_oberman_1 method
% Algebraic decay far-field integrator
...
% Theoretical error y = 0.0112 * h^(1.6)
\addplot [domain=8e-4:0.8, samples=11, color=gray, line width=6pt, dashed] {0.0112*x^(1.6)};
\addlegendentry{$O(h^{2-\alpha})$};

% Read the file with the datatable:
\pgfplotstableread{max-abs-errs-farfield-algebraic.dat}{\datatable};
