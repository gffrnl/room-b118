...
#include <b118/fftw/complex.hpp>
#include <b118/fftw/complex_ptr.hpp>
#include <b118/fftw/alloc.hpp>
#include <b118/fftw/free.hpp>
#include <b118/fftw/dft.hpp>
...
class convolution final : private b118::fftw::managed<Real> {
    ...
    Real * kernel;
    Real * out;
    b118::fftw::complex_ptr<Real> in_fft;
    b118::fftw::complex_ptr<Real> kernel_fft;
    b118::fftw::plan<Real> plan_r2c;
    b118::fftw::plan<Real> plan_c2r;
    ...
    void elementwise_product(bool conj = false) {
        if (conj == false) {
            for (std::size_t i = 0; i < conv_size/2 + 1; ++i) {
                Real re = + in_fft.raw[i][0] * kernel_fft.raw[i][0]
                            - in_fft.raw[i][1] * kernel_fft.raw[i][1];
                Real im = + in_fft.raw[i][0] * kernel_fft.raw[i][1]
                            + in_fft.raw[i][1] * kernel_fft.raw[i][0];
                in_fft.raw[i][0] = re;
                in_fft.raw[i][1] = im;
            }
        } else {
 ...
 public:
    explicit convolution(std::size_t N) : capacity(N) {
        ...
        in         = b118::fftw::alloc<Real>(N);
        kernel     = b118::fftw::alloc<Real>(N);
        out        = b118::fftw::alloc<Real>(N);
        in_fft     = b118::fftw::alloc_complex<Real>(N/2 + 1);
        kernel_fft = b118::fftw::alloc_complex<Real>(N/2 + 1);
    }
    ...
    ~convolution() {
        ...
        b118::fftw::free(in);
        b118::fftw::free(kernel);
        b118::fftw::free(out);
        b118::fftw::free_complex(in_fft);
        b118::fftw::free_complex(kernel_fft);
    }
