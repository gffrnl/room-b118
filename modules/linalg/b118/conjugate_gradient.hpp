...
std::size_t conjugate_gradient(
    ...
    ublas::vector<T> p(r);
    ublas::vector<T> Ap(b.size());
    std::size_t k = 1;
    for (;;) {
        T const rTr = ublas::inner_prod(r,r);
        ublas::noalias(Ap) = ublas::prod(A,p);
        T const alpha = rTr / ublas::inner_prod(r,Ap);
        x += alpha * p;
        r -= alpha * Ap;
        if (sup_norm(r) < tol) return k;
        T const beta = ublas::inner_prod(r,r) / rTr;
        p = r + beta * p;
        ++k;
    }
